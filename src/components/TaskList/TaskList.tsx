...
function TaskList() {
  ...
  tasks.data.map((task) => task.date && dates.add(task.date.toString()));
  /* if (dates.size !== 0) {
    dates.forEach((value) => console.log(new Date(value)));
  } */
  const daysComponents: JSX.Element[] = [];
  for (const value of dates) daysComponents.push(<DateItem key={value} date={value} />);
  return (
